...
def mechanics_dashboard():
    ...
    if action == "Update Mechanic":
        ...
        if st.button("Update Mechanic"):
            ...
            update_mechanic(api_key, mechanic_id, new_name)
        ...
        if st.button("Delete Mechanic"):
            ...
            delete_mechanic(api_key, mechanic_id)

def cars_dashboard(api_key):
    ...
    if action == "Update Car":
        selected_book = st.selectbox("Select Car to Update", options=[car['title'] for car in cars],
                                     key="select_car_update")

        if selected_book:
            car = next((book for book in cars if book['title'] == selected_book), None)
            new_car_make = st.text_input("Title", value=car['title'])
            selected_mechanic_name = st.selectbox("Select Author", options=[author['name'] for author in mechanics],
                                                index=[author['name'] for author in mechanics].index(car['author']),
                                                key="select_author_update")
            new_book_average_rating = st.number_input("Average Rating", min_value=0.0, max_value=5.0, step=0.01,
                                                      value=car['average_rating'])
            new_book_genres = st.text_input("Genres (comma-separated names)",
                                            value=car['genres'])  # Show existing genres as a comma-separated string
            new_car_year = st.number_input("Year", min_value=1440, max_value=datetime.now().year, step=1,
                                            value=car['published_year'])
            car_id = car['id']
            ...
            if st.button("Update Book"):
                ...
                car_data = {
                    "title": new_car_make,
                    "author_id": next((author['id'] for author in mechanics if author['name'] == selected_mechanic_name),
                                      None),
                    "book_link": car.get('book_link', ""),
                    "genres": genres_list,  # Update with the list of genre names
                    "average_rating": new_book_average_rating,
                    "published_year": new_car_year
                }
                update_car(api_key, car_id, car_data)

    elif action == "Delete Car":
        car_to_delete = st.selectbox("Select Car to Delete", options=[car['make'] for car in cars], key="select_book_delete")
        if st.button("Delete Book"):
            car_id = next((car['id'] for car in cars if car['make'] == car_to_delete), None)
            delete_car(api_key, car_id)
